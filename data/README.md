...
2. **Labelling**: intent was labelled by an independent keyword heuristic (`scripts/build_golden_set.py`) -- deliberately *not* the classifier under test, to keep the eval non-circular. It disagreed with the ingestion-time classifier on **41.4%** of the pool (412 of 995), itself a signal of how ambiguous this domain is. Triage action came from a separate hazard/PII/legal detector, then **all 9 real candidate-ESCALATE cases were hand-reviewed**, since the first-pass regex had real false positives (a customer *reporting* a scam, a case number matching a phone regex, hyperbolic "IM SUING"). Result: **5** disclosed hard negatives, 2 genuine escalations, 1 ambiguous case that's the real-world justification for the third triage action (`CLARIFY`), and **1 candidate dropped from the golden set entirely** (`kaggle_359548_359547`, "what do I do if I've been sent a scam email?" -- a question about a scam rather than a report of one). That accounts for all 9. *This paragraph said "4 disclosed hard negatives" and accounted for only 7 of the 9 until adversarial review round 3 counted the rows.*
