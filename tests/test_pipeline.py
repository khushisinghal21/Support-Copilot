...
def test_pipeline_single_tweet_default_execution():
    ...
    tweet = TweetInput(tweet_id="12345", text="My iPhone 11 Wi-Fi keeps dropping.", author_id="user_1")
...
def test_pipeline_batch_processing():
    pipeline = SupportPipeline()
    tweets = [TweetInput(tweet_id=f"tw_{i}", text=f"Sample issue {i}", author_id=f"author_{i}") for i in range(3)]
...
def test_pipeline_fail_closed_circuit_breaker():
    ...
    tweet = TweetInput(tweet_id="err_tweet", text="Can you help me?", author_id="user_err")
    response = pipeline.process(tweet)
