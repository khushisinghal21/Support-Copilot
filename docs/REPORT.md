...
| **ROUGE-L Grounding Score** | 0.1572 | 0.1465 | **0.2028** | **+0.0563** |
| **LLM Judge Quality (1-5 Scale)** | 4.3 / 5.0 | 4.0 / 5.0 | **4.1 / 5.0** | **+0.1** |
| **P95 Latency (CPU)** | < 1 ms (unmeasured estimate) | ~5 ms (unmeasured estimate) | **39.1 ms** | Real-time ready |
...
> **On the precision of these numbers.** Escalation recall is 90.5% because the system caught 19 of 21 true escalations in the held-out set. With n = 21, the 95% Wilson interval is **71.1% - 97.3%** -- the data are consistent with materially worse real performance, and '2 missed' is a draw from a wide distribution rather than a property of the system. The *direction* is solid (Fisher exact against the TF-IDF baseline gives p < 0.001 on both escalation recall and intent accuracy); the trailing digit is not. Quote the interval, not the point estimate.
